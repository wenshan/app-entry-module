...
class Navi extends React.Component {
    ...
    generateMenuDom(menus, path) {
        ...
        menus.forEach((item) => {
            ...
            var hash = item.key.length ? this.props.suffix : '';
            var toObj = {
                pathname: p + hash
            };
            if (item.visible !== false) {
                if (this.hasVisibleChildren(item)) {
                    menuDoms.push(<SubMenu key={item.key}
                                           title={item.title}>{this.generateMenuDom(item.children, p)}</SubMenu>)
                } else {
                    if (item.component) {
                        menuDoms.push(<Item key={item.key}><Link to={toObj}>{item.title}</Link></Item>)
                    } else {
                        menuDoms.push(<Item disabled={true} key={item.key}>{item.title}</Item>)
                    }
                }
            }
        });
